...
class Effective_Ranks_GPU:
    def __init__(self, x):
        ...
        Args:
        - x: Input matrix as a NumPy array or PyTorch tensor.
        """
        if isinstance(x, np.ndarray):
            x = torch.tensor(x, dtype=torch.float32, device='cuda')  # Convert to PyTorch tensor and move to GPU
    ...
    def get_cov_operator(self) -> None:
        ...
        """
        if self.x.size(0) > self.x.size(1):  # Check if m > n
            self.x = self.x.t()  # Transpose self.x
