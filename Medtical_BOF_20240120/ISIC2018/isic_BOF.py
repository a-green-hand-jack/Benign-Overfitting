...
class ISICBOF:
    ...
    def images2matrix(self, trainset: torch.utils.data.Dataset) -> torch.Tensor:
        """
        Convert images in the dataset to matrices.
        ...
        Returns:
        - Image matrix as PyTorch tensor.
        """
        print(len(trainset))
        dataloader = torch.utils.data.DataLoader(trainset, batch_size=128, shuffle=False)  # Adjust batch_size as needed
        image_vectors = []

        for images, _, _ in dataloader:
            images_numpy = images # 将PyTorch张量转换为NumPy数组
            image_vectors.append(images_numpy.reshape(images_numpy.shape[0], -1))

        image_matrix = torch.cat(image_vectors, dim=0)
        print(image_matrix.shape)
        return image_matrix.detach().cpu().numpy() 
