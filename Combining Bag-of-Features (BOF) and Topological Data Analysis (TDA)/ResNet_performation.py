...
def scale_resnet_pre(scale_path = "./Result/ResNetAcc", model=ResNet18(), model_name="ResNet18", aug_name="scale"):
    ...
    scale_list = np.arange(0.1, 0.18, 0.02)
...
def all_resnet(aug_name):
    ...
    if aug_name == "angle":
        for model, model_name in zip(model_list, model_names):
            ...
            angle_resnet_pre(scale_path="./Result/ResNetAcc", model=model, model_name=model_name, aug_name="angle")
    elif aug_name == "scale":
        for model, model_name in zip(model_list, model_names):
            print("-"*10, f"现在是{model_name}下的考察", "-"*10)
            scale_resnet_pre(scale_path="./Result/ResNetAcc", model=model, model_name=model_name, aug_name="scale")
