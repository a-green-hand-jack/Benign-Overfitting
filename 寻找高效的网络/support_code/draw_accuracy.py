...
def draw_accuracy(output_dir="csv文件所在位置"):
    '''
...
def draw_acc_change(output_dir="csv文件所在位置"):
    """"
    ...
    for i, csv_file in enumerate(csv_files):
        ...
        # 保存子图
        output_filename = os.path.join(output_dir, f"{csv_file.split('.csv')[0]}.png")  # 文件名与题目一致，去除扩展名并添加.png后缀
        plt.savefig(output_filename)
        print(f"Plot saved as '{output_filename}'")


def best_valid_acc(output_dir="csv文件所在位置"):
    # 列出包含数据的CSV文件
    csv_files = [file for file in os.listdir(output_dir) if file.endswith('.csv')]
    ...
    for csv_file in csv_files:
        # 判断文件名是否包含"valid"
        if "valid" in csv_file:
            # 获取scale的索引位置
            # scale_index = int(csv_file.split('=')[1].split('.csv')[0])  # 提取scale的索引
            scale_key = csv_file.split("_")[1].split(".csv")[0]
    ...
    # 绘制子图
    plt.plot(max_values.keys(), max_values.values(), marker='o')
    ...
    # 保存子图
    output_filename = os.path.join(output_dir, "观察valid中最大准确率的变化.png")  # 文件名与题目一致，去除扩展名并添加.png后缀
    plt.savefig(output_filename)
    print(f"Plot saved as '{output_filename}'")